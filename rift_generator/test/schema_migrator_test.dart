import 'package:rift_generator/src/builder/schema_migrator_builder.dart';
import 'package:test/test.dart';

import 'test_utils.dart';

const buildYaml = {
  'build.yaml': r'''
targets:
  $default:
    builders:
      hive_ce_generator|rift_schema_migrator:
        enabled: true
''',
};

const adapters = {
  'lib/adapters.dart': '''
import 'package:rift/rift.dart';

@RiftType(typeId: 1)
class Class1 {}

@RiftType(typeId: 0)
class Class2 {
  const Class2(this.lastName, this.firstName);

  @RiftField(1)
  final String lastName;

  @RiftField(0)
  final String firstName;
}
''',
  'lib/adapters_2.dart': '''
import 'package:rift/rift.dart';

@RiftType(typeId: 3)
class Class3 {
  Class3({required double value}): _value = value;

  @RiftField(0)
  double _value;

  double get value => _value;
}

@RiftType(typeId: 4)
enum Enum {
  @RiftField(0)
  a,

  @RiftField(1)
  b,

  @RiftField(2)
  c,
}
''',
};

void main() {
  group('schema_migrator', () {
    test('does not run if not enabled', () {
      expectGeneration(
        input: {
          ...pubspec(),
          ...adapters,
        },
        output: {
          'lib/rift_schema.g.yaml': fileDoesNotExist,
        },
      );
    });

    test('generates schema', () {
      expectGeneration(
        input: {
          ...pubspec(),
          ...buildYaml,
          ...adapters,
        },
        output: {
          'lib/hive/rift_adapters.dart': '''
import 'package:rift/rift.dart';
import 'package:rift_generator_test/adapters.dart';
import 'package:rift_generator_test/adapters_2.dart';

@GenerateAdapters([
  AdapterSpec<Class2>(),
  AdapterSpec<Class1>(),
  AdapterSpec<Class3>(),
  AdapterSpec<Enum>(),
])
part 'rift_adapters.g.dart';
''',
          'lib/hive/rift_adapters.g.yaml': '''
$schemaComment
nextTypeId: 5
types:
  Class2:
    typeId: 0
    nextIndex: 2
    fields:
      firstName:
        index: 0
      lastName:
        index: 1
  Class1:
    typeId: 1
    nextIndex: 0
    fields: {}
  Class3:
    typeId: 3
    nextIndex: 1
    fields:
      value:
        index: 0
  Enum:
    typeId: 4
    nextIndex: 3
    fields:
      a:
        index: 0
      b:
        index: 1
      c:
        index: 2
''',
        },
      );
    });

    test('throws with default value', () {
      expectGeneration(
        input: {
          ...pubspec(),
          ...buildYaml,
          'lib/adapters.dart': '''
import 'package:rift/rift.dart';

@RiftType(typeId: 0)
class Class {
  @RiftField(0, defaultValue: 42)
  int? value;
}
''',
        },
        throws: RiftSchemaMigratorBuilder.hasAnnotationDefaultValue(
          className: 'Class',
          fieldName: 'value',
        ),
      );
    });

    test('throws with no public setter', () {
      expectGeneration(
        input: {
          ...pubspec(),
          ...buildYaml,
          'lib/adapters.dart': '''
import 'package:rift/rift.dart';

@RiftType(typeId: 0)
class Class {
  @RiftField(0)
  int? _value;

  int? get value => _value;
}
''',
        },
        throws: RiftSchemaMigratorBuilder.hasNoPublicSetter(
          className: 'Class',
          fieldName: '_value',
        ),
      );
    });

    test('throws with no public getter', () {
      expectGeneration(
        input: {
          ...pubspec(),
          ...buildYaml,
          'lib/adapters.dart': '''
import 'package:rift/rift.dart';

@RiftType(typeId: 0)
class Class {
  Class({required int value}): _value = value;

  @RiftField(0)
  int? _value;
}
''',
        },
        throws: RiftSchemaMigratorBuilder.hasNoPublicGetter(
          className: 'Class',
          fieldName: '_value',
        ),
      );
    });

    test('adds unannotated fields to ignored fields', () {
      expectGeneration(
        input: {
          ...pubspec(),
          ...buildYaml,
          'lib/adapters.dart': '''
import 'package:rift/rift.dart';

@RiftType(typeId: 0)
class Class {
  @RiftField(0)
  int? value;

  int? value2;
}
''',
        },
        output: {
          'lib/hive/rift_adapters.dart': '''
import 'package:rift/rift.dart';
import 'package:rift_generator_test/adapters.dart';

@GenerateAdapters([
  AdapterSpec<Class>(ignoredFields: {'value2'}),
])
part 'rift_adapters.g.dart';
''',
        },
      );
    });

    test(
      'works with freezed classes',
      () {
        expectGeneration(
          input: {
            ...pubspec(
              dependencies: {'freezed_annotation: any'},
              devDependencies: {'freezed: any'},
            ),
            ...buildYaml,
            'lib/adapters.dart': '''
import 'package:rift/rift.dart';
import 'package:freezed_annotation/freezed_annotation.dart';

part 'adapters.freezed.dart';
part 'adapters.g.dart';

@RiftType(typeId: 0)
@freezed
sealed class Class with _\$Class {
  factory Class({@RiftField(0) required int value}) = _Class;
}
''',
          },
          output: {
            'lib/hive/rift_adapters.dart': '''
import 'package:rift/rift.dart';
import 'package:rift_generator_test/adapters.dart';

@GenerateAdapters([
  AdapterSpec<Class>(),
])
part 'rift_adapters.g.dart';
''',
            'lib/hive/rift_adapters.g.yaml': '''
$schemaComment
nextTypeId: 1
types:
  Class:
    typeId: 0
    nextIndex: 1
    fields:
      value:
        index: 0
''',
          },
        );
      },
      skip: 'Waiting on freezed analyzer 9 suport',
    );
  });
}
