...
class BinaryWriterImpl extends BinaryWriter {
  ...
  void writeInt(int value) {
    // Use ZigZag + varint encoding for full 64-bit integer precision.
    // This avoids precision loss for values > 2^53 that occurred with
    // the old float64 encoding.
    _writeVarint(_encodeZigZag(value));
  }

  /// Encode a signed integer using ZigZag encoding.
  /// Maps signed integers to unsigned so that small absolute values
  /// have small varint encodings.
  /// - 0 → 0, -1 → 1, 1 → 2, -2 → 3, 2 → 4, ...
  int _encodeZigZag(int n) {
    return (n << 1) ^ (n >> 63);
  }
  ...
  void writeIntList(List<int> list, {bool writeLength = true}) {
    ...
    for (var i = 0; i < length; i++) {
      writeInt(list[i]); // Uses varint + ZigZag encoding
    }
  }
  ...
  @override
  void writeRiftList(RiftList list, {bool writeLength = true}) {
    if (writeLength) {
      writeUint32(list.length);
    }
    final boxName = (list as RiftListImpl).boxName;
  ...
  void write<T>(T value, {bool withTypeId = true}) {
    ...
    if (value == null) {
      ...
      typeId = FrameValueType.intVarintT;
  ...
  void _writeList(Iterable value, {bool withTypeId = true}) {
    final int typeId;
    final void Function() write;
    if (value is RiftList) {
      typeId = FrameValueType.riftListT;
      write = () => writeRiftList(value);
      ...
      throw RiftError('Cannot write, unknown type: ${value.runtimeType}.');
    }
