...
import 'package:rift/src/box_collection/box_collection_stub.dart'
    as implementation;
...
class BoxCollection implements implementation.BoxCollection {
  ...
  static Future<BoxCollection> open(
    ...
    if ((Rift as RiftImpl).homePath == null) {
      Rift.init(path ?? './');
    }
    ...
    collection._badKeyBox = await Rift.openBox<String>('${name}_bad_keys');
  ...
  Future<CollectionBox<V>> openBox<V>(
    ...
    final box =
        boxCreator?.call(boxIdentifier, this) as CollectionBox<V>? ??
        CollectionBox<V>(boxIdentifier, this);
    if (preload) {
      box._cachedBox = await Rift.openBox(
        box.name,
  ...
  Future<void> deleteFromDisk() =>
      Future.wait(boxNames.map(Rift.deleteBoxFromDisk));
}
...
class CollectionBox<V> implements implementation.CollectionBox<V> {
  ...
  Future<BoxBase> _getBox() async {
    return _cachedBox ??= await Rift.openLazyBox<V>(
      name,
  ...
  Future<Map<String, V>> getAllValues() async {
    ...
    return (box as Box).toMap().map(
      (k, v) => MapEntry(Uri.decodeComponent(k.toString()), v as V),
    );
  }
  ...
  @override
  Future<List<V?>> getAll(List<String> keys) async {
    final box = await _getBox();
    final values = <V?>[];
    for (var key in keys) {
      key = _toRiftKey(key);
  ...
  Future<void> put(String key, V val, [Object? transaction]) async {
    ...
    final box = await _getBox();
    await box.put(_toRiftKey(key), val);
    await _flushOrMark();
  }
  ...
  Future<void> delete(String key) async {
    final box = await _getBox();
    await box.delete(_toRiftKey(key));
    await _flushOrMark();
  }

  @override
  Future<void> deleteAll(List<String> keys) async {
    final riftKeys = keys.map(_toRiftKey);
    final box = await _getBox();
    await box.deleteAll(riftKeys);
    await _flushOrMark();
  }
  ...
  Future<void> flush() async {
    ...
    // other stuff while the flusing is still in progress. Fortunately, Rift has
