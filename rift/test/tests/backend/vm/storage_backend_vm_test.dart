  ...
  group('StorageBackendVm', () {
    ...
    group('.open()', () {
      ...
      test('writeOffset', () async {
        ...
        when(file.open).thenAnswer((i) => Future.value(readRaf));
        when(writeRaf.length).thenAnswer((i) => Future.value(123));
        when(
          () => readRaf.setPosition(0),
        ).thenAnswer((i) => Future.value(readRaf));
        when(
          () => readRaf.read(any()),
        ).thenAnswer((i) => Future.value(Uint8List(0)));
    ...
    group('.initialize()', () {
      ...
      FrameIoHelper getFrameIoHelper(int recoveryOffset) {
        ...
        when(
          () => helper.framesFromFile(any(), any(), any(), any(), any()),
        ).thenAnswer((i) => Future.value(recoveryOffset));
        when(
          () => helper.keysFromFile(any(), any(), any(), any()),
        ).thenAnswer((i) => Future.value(recoveryOffset));
        return helper;
      }
    ...
    /*group('.compact()', () {
      ...
      test('throws error if corrupted', () async {
        ...
        var box = BoxImplVm(
            RiftImpl(), path.basename(boxFile.path), BoxOptions(), syncedFile);
        ...
        expect(() => box.compact(), throwsRiftError(['unexpected eof']));
      });
    });*/
    ...
    test('.clear()', () async {
      ...
      verify(() => writeRaf.writeFrom(any()));
      expect(backend.writeOffset, 17); // FileHeader.headerSize
    });
