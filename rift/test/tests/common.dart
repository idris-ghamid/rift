...
Matcher RiftErrorPredicate(List<String> containing) => predicate((e) {
  final message = e.toString().toLowerCase();
  return containing.every((c) => message.contains(c.toLowerCase()));
});

Matcher isARiftError([List<String> containing = const []]) {
  return allOf(isA<RiftError>(), RiftErrorPredicate(containing));
}

Matcher throwsRiftError([List<String> containing = const []]) {
  return throwsA(isARiftError(containing));
}

Matcher throwsIsolatedRiftError([List<String> containing = const []]) {
  return throwsA(
    anyOf([
      isARiftError(containing),
      allOf(isA<IsolateException>(), RiftErrorPredicate(containing)),
    ]),
  );
}
