...
import '../frames.dart';
import '../mocks.dart' as test_mocks;
...
void main() {
  group('BinaryWriter', () {
    ...
    test('.writeInt()', () async {
      ...
      expect(output2, contains(RiftWarning.bigInt));
    ...
    group('.writeRiftList()', () {
      final box = test_mocks.MockBox();
      when(() => box.name).thenReturn('Box');

      test('write length', () {
        final list = RiftList(box);
        final bw = getWriter();
        bw.writeRiftList(list);

        expect(bw.toBytes(), [
          0, 0, 0, 0, 3, 66, 111, 120, //
        ]);
      });
      ...
      test('omit length', () {
        final list = RiftList(box);
        final bw = getWriter();
        bw.writeRiftList(list, writeLength: false);
