...
import 'package:rift_generator/src/model/revived_generate_adapter.dart';
import 'package:rift_generator/src/generator/type_adapter_generator.dart';
import 'package:source_gen/source_gen.dart';
import 'package:yaml/yaml.dart';

/// Builder that generates Rift adapters from a GenerateAdapters annotation
class AdaptersGenerator extends GeneratorForAnnotation<GenerateAdapters> {
  ...
  Future<String> _generate(
    ...
    final RiftSchema schema;
    if (await buildStep.canRead(schemaAsset)) {
      final schemaContent = await buildStep.readAsString(schemaAsset);
      schema =
          RiftSchema.fromJson(jsonDecode(jsonEncode(loadYaml(schemaContent))));
    } else {
      schema = RiftSchema(nextTypeId: revived.firstTypeId, types: {});
    }
    ...
    final newTypes = <String, RiftSchemaType>{};
    ...
    buildStep.forceWriteAsString(
      schemaAsset,
      writeSchema(RiftSchema(nextTypeId: typeId + 1, types: newTypes)),
    );

    return content.toString();
  }

  void _validateSchema(RiftSchema schema) {
    void invalidSchema(String message) {
      throw RiftError('Invalid schema: $message');
    }
