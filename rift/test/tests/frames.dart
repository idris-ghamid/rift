...
TypeRegistry get testRegistry => RiftImpl();

class _RiftAesCipherStaticIV extends RiftAesCipher {
  _RiftAesCipherStaticIV() : super(Uint8List.fromList(List.filled(32, 1)));

  @override
  Uint8List generateIv() => Uint8List.fromList(List.filled(16, 4));
}

RiftCipher get testCipher => _RiftAesCipherStaticIV();
...
Future<void> buildGoldens() async {
  ...
  await generate('frame_values', 'frameValuesBytes', (f) {
    final writer = BinaryWriterImpl(RiftImpl())..write(f.value);
    return writer.toBytes();
  });
  ...
  await generate('frame_values_encrypted', 'frameValuesBytesEncrypted', (f) {
    final writer = BinaryWriterImpl(RiftImpl())
