...
void main() {
  group('IsolatedHive', () {
    Future<IsolatedRiftImpl> initHive() async {
    ...
    group('.openBox()', () {
      group('box already open', () {
        ...
        test('throw RiftError if opened box is lazy', () async {
          final hive = await initHive();
        ...
        test('throw RiftError if already opening box is lazy', () async {
          final hive = await initHive();
    ...
    group('.openLazyBox()', () {
      group('box already open', () {
        ...
        test('throw RiftError if opened box is not lazy', () async {
          final hive = await initHive();

          await hive.openBox('LAZYBOX');
          await expectLater(
            () => hive.openLazyBox('lazyBox'),
            throwsRiftError(['is already open and of type', 'Box<dynamic>']),
          );

          await hive.close();
        });

        test('throw RiftError if already opening box is not lazy', () async {
          final hive = await initHive();
    ...
    group('.box()', () {
      test('returns already opened box', () async {
        ...
        expect(() => hive.box('other'), throwsRiftError(['not found']));

        await hive.close();
      });

      test('throws RiftError if box type does not match', () async {
        final hive = await initHive();

        await hive.openBox<int>('INTBOX');
        expect(
          () => hive.box('intBox'),
          throwsRiftError(['is already open and of type', 'Box<int>']),
        );
        ...
        expect(
          () => hive.box('lazyBox'),
          throwsRiftError(['is already open and of type', 'LazyBox<dynamic>']),
        );
    ...
    group('.lazyBox()', () {
      test('returns already opened box', () async {
        ...
        expect(() => hive.lazyBox('other'), throwsRiftError(['not found']));
      ...
      test('throws RiftError if box type does not match', () async {
        ...
        expect(
          () => hive.lazyBox('intBox'),
          throwsRiftError(['is already open and of type', 'LazyBox<int>']),
        );
