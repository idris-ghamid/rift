  ...
  static bool assertKey(dynamic key) {
    if (key is int) {
      if (key < 0 || key > 0xFFFFFFFF) {
        throw RiftError('Integer keys need to be in the range 0 - 0xFFFFFFFF');
      }
    } else if (key is String) {
      if (key.length > 0xFF) {
        throw RiftError('String keys need to be a max length of 255');
      }
    } else {
      throw RiftError('Keys need to be Strings or integers');
    }
...
final class FrameValueType {
  ...
  /// Type ID extension
  ///
  /// Informs Rift that the next 2 bytes contain the real type ID
  static const typeIdExtension = 21;

  /// `int` encoded as varint (ZigZag + varint for full 64-bit precision)
  static const intVarintT = 22;
}
