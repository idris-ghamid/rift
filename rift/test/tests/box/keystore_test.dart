  ...
  group('Keystore', () {
    ...
    group('.insert()', () {
      group('add', () {
        ...
        test('initializes RiftObject', () {
          final box = MockBox();
          final keystore = Keystore.debug(box: box);

          final RiftObject = TestRiftObject();
          keystore.insert(Frame('key', RiftObject));

          expect(RiftObject.key, 'key');
          expect(RiftObject.box, box);
        });
        ...
        test('unloads previous RiftObject', () {
          final box = MockBox();
          final keystore = Keystore.debug(box: box);

          final RiftObject = TestRiftObject();
          keystore.insert(Frame('key', RiftObject));
          keystore.insert(Frame('key', TestRiftObject()));

          expect(RiftObject.key, null);
          expect(RiftObject.box, null);
        });

        test('does not unload RiftObject if it is the same instance', () {
          final box = MockBox();
          final keystore = Keystore.debug(box: box);
      ...
      group('delete', () {
        ...
        test('unloads deleted RiftObject', () {
          final box = MockBox();
          final RiftObject = TestRiftObject();
          final keystore = Keystore.debug(
            frames: [Frame('key', RiftObject)],
            box: box,
          );
    ...
    group('.clear()', () {
      ...
      test('unloads RiftObjects', () {
        final RiftObject = TestRiftObject();
        final box = MockBox();
        final keystore = Keystore.debug(
          frames: [Frame('key1', 'val1'), Frame('key2', RiftObject)],
          box: box,
        );
        expect(RiftObject.key, 'key2');
        expect(RiftObject.box, box);
