...
class _TestObject with RiftObjectMixin {
  String name;
...
Future _performTest(bool lazy) async {
  ...
  await rift.registerAdapter<_TestObject>(_TestObjectAdapter());
  var (_, box) = await openBox(lazy, type: TestType.normal, rift: rift);
...
void main() {
  group('use RiftObject to update and delete entries', () {
    test('normal box', () => _performTest(false));

    test('lazy box', () => _performTest(true));
  }, timeout: longTimeout);

  test('move RiftObject between lazy boxes', () async {
    final rift = await createHive(type: TestType.normal);
    await rift.registerAdapter<_TestObject>(_TestObjectAdapter());

    final (_, box1) = await openBox(true, type: TestType.normal, rift: rift);
    final (_, box2) = await openBox(true, type: TestType.normal, rift: rift);
