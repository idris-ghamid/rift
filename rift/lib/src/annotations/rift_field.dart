import 'package:meta/meta_meta.dart';
import 'package:meta/meta.dart';

/// Annotate all fields you want to persist with [RiftField].
@Target({
  TargetKind.field,
  TargetKind.getter,
  TargetKind.setter,
  TargetKind.enumValue,
  TargetKind.parameter,
})
@immutable
class RiftField {
  /// The index of this field.
  final int index;

  /// The default value of this field for class Rift types.
  ///
  /// This value takes precedence over constructor parameter default values.
  ///
  /// In enum Rift types set `true` to use this enum value as default value
  /// instead of null in null-safety.
  ///
  /// ```dart
  /// @RiftType(typeId: 1)
  /// enum MyEnum {
  ///   @RiftField(0)
  ///   apple,
  ///
  ///   @RiftField(1, defaultValue: true)
  ///   pear
  /// }
  /// ```
  final dynamic defaultValue;

  const RiftField(this.index, {this.defaultValue});
}
