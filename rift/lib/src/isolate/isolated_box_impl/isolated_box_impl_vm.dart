...
import 'package:rift/src/isolate/isolated_rift_impl/impl/isolated_rift_impl_vm.dart';
...
abstract class IsolatedBoxBaseImpl<E>
  ...
  static const defaultValuePlaceholder = '_rift.defaultValue';

  final IsolatedRiftImpl _rift;
  ...
  Stream<BoxEvent> watch({dynamic key}) {
    final stream = _stream ??= _eventChannel
  ...
  @override
  Future<void> put(dynamic key, E value) => _channel.invokeMethod('put', {
    'name': name,
    'key': key,
  ...
  @override
  Future<void> putAt(int index, E value) => _channel.invokeMethod('putAt', {
    'name': name,
    'index': index,
    'value': _writeValue(value),
  });
  ...
  Future<void> putAll(Map<dynamic, E> entries) =>
      _channel.invokeMethod('putAll', {
        'name': name,
        'entries': entries.map(
          (key, value) => MapEntry(key, _writeValue(value)),
        ),
      });
  ...
  Future<void> close() async {
    ...
    _open = false;
    await _rift.unregisterBox(name);
  ...
  Future<E?> getAt(int index) async {
    final bytes = await _channel.invokeMethod('getAt', {
      'name': name,
      'index': index,
    });
  ...
  Uint8List _writeValue(E value) {
    final writer = BinaryWriterImpl(_rift);
    if (_cipher != null) {
  ...
  E? _readValue(Uint8List? bytes) {
    ...
    final reader = BinaryReaderImpl(bytes, _rift);
    final E? value;
    if (_cipher != null) {
  ...
  @override
  TypeRegistry get typeRegistry => _rift;

  @override
  Future<Iterable<InspectorFrame>> getFrames() async {
    final result = await _channel.invokeListMethod<Map>('getFrames', {
      'name': name,
    });
    return result
...
class IsolatedBoxImpl<E> extends IsolatedBoxBaseImpl<E>
  ...
  Future<Iterable<E>> get values async {
    final bytes = await _channel.invokeListMethod<Uint8List>('values', {
      'name': name,
    });
  ...
  Future<Iterable<E>> valuesBetween({dynamic startKey, dynamic endKey}) async {
    final bytes = await _channel.invokeListMethod<Uint8List>('valuesBetween', {
      'name': name,
      'startKey': startKey,
      'endKey': endKey,
    });
  ...
  Future<Map<dynamic, E>> toMap() async {
    final bytes = await _channel.invokeMapMethod<dynamic, Uint8List>('toMap', {
      'name': name,
    });
