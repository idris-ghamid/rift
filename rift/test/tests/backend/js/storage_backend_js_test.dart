...
import 'package:rift/src/backend/js/native/storage_backend_js.dart';
import 'package:rift/src/backend/js/native/utils.dart';
...
void main() async {
  ...
  group('StorageBackendJs', () {
    ...
    group('.decodeValue()', () {
      ...
      test('crypto', () {
        final cipher = RiftAesCipher(Uint8List.fromList(List.filled(32, 1)));
        ...
        for (final testFrame in testFrames) {
          ...
          final value = backend.decodeValue(
    ...
    group('.initialize()', () {
      test('not lazy', () async {
        ...
        expect(
          await backend.initialize(TypeRegistryImpl.nullImpl, keystore, false),
          0,
        );
    ...
    test('.compact()', () async {
      final db = await _getDbWith({});
      final backend = _getBackend(db: db);
      expect(() async => await backend.compact({}), throwsUnsupportedError);
    });
